...
async function walk(dir) {
  ...
  for (const e of await readdir(dir, { withFileTypes: true })) {
    if (e.isDirectory()) r.push(...(await walk(`${dir}/${e.name}`)));
    else r.push(`${dir}/${e.name}`);
  }
  return r;
}
...
const name = "fl-fave-" + hash.digest("hex").slice(0, 16);
const urls = ["/", ...files.map((f) => "/" + f.slice(4))];
await writeFile(
  "out/sw.js",
  `const CACHE=${JSON.stringify(name)};const ASSETS=${JSON.stringify(urls)};
